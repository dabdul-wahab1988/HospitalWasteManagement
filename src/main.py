...
# Configure logging once at the top of the module.
...
def main():
    ...
    scenarios = {
        "BASELINE": {
            ...
            "incineration_flue_gas_efficiency": 0.2,
        ...
        "POLICY_NETZERO": {
            ...
            "energy_policy_factor": 0.7,
            "landfill_best_practices": True,
            "biomedical_risk_factor": 0.01,
            "chemical_disinfection_fraction": 0.000001
        },
        "WORST_CASE": {
            ...
            "incineration_flue_gas_efficiency": 0.1,
            "energy_policy_factor": 0.1,
            "landfill_best_practices": False,
            ...
            for process_key, process_obj in processes.items():
                ...
                if indirect_calc:
                    ...
                    for key, val in indirect_emissions.items():
                        # Ensure the indirect value is a Pint quantity. If not, assume its unit is kg.
                        if not isinstance(val, ureg.Quantity):
                            val = ureg.Quantity(val, "kg")
                        if key in direct_emissions:
                            direct_val = direct_emissions[key]
                            # If direct_val is not a Pint quantity, assume its unit is kg.
                            if not isinstance(direct_val, ureg.Quantity):
                                direct_val = ureg.Quantity(direct_val, "kg")
                            try:
                                # Convert indirect value to the same unit as direct_val.
                                converted_val = val.to(direct_val.units)
                            except Exception:
                                converted_val = ureg.Quantity(val.magnitude, "kg")
                            direct_emissions[key] = direct_val + converted_val
                        else:
                            # For keys not already in direct_emissions, assign them with an expected unit.
                            if key in ["co2_fossil", "so2", "pm25", "nox", "hg", "pb"]:
                                direct_emissions[key] = val.to("kg")
                            elif key == "land_occupation":
                                direct_emissions[key] = val.to("meter**2*year")
                            else:
